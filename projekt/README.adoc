...
Pod adresem `https://arcane-spire-76209.herokuapp.com/services/ApiKeys?wsdl` znajduje się serwis udostępniający w czasie _rzeczywistym_ klucze potrzebne do skorzystania z serwisy tłumaczącego Google.
...
.Algorytm działania
. W pierwszy kroku musimy zweryfikować czy zdanie zawiera przekleń - wywołując serwis `/profranity/`
. Następnie tłumaczymy przekleństwo na polski (wykorzystując słownik dict.pl)
